...
class TestMDSs(object):
    ...
    def test_mds_is_up(self, node, host):
        hostname = node["vars"]["inventory_hostname"]
        cmd = "sudo ceph --name client.bootstrap-mds --keyring /var/lib/ceph/bootstrap-mds/{cluster}.keyring --cluster={cluster} --connect-timeout 5 -f json -s".format(  # noqa E501
    ...
    def test_docker_mds_is_up(self, node, host):
        cmd = "sudo docker exec ceph-mds-{hostname} ceph --name client.bootstrap-mds --keyring /var/lib/ceph/bootstrap-mds/{cluster}.keyring --cluster={cluster} --connect-timeout 5 -f json -s".format(  # noqa E501
            hostname=node["vars"]["inventory_hostname"],
            cluster=node["cluster_name"]
        )
